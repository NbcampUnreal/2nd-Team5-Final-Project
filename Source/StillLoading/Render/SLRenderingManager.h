...
struct FBaseOutlineSettings
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SLRendering|Outline|Distance", DisplayName = "최대 뎁스 거리", meta = (ClampMin = "500.0", ClampMax = "20000.0"))
	float MaxDepthRange = 10000.0f;
	
	// SceneDepth 기준 - 최소 깊이 거리입니다. 
	// 이 값이 MaxDepthRange에 가까울수록 아웃라인이 적용되는 거리 범위가 좁아지고, 멀리 있는 물체 위주로 강조됩니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SLRendering|Outline|Distance", DisplayName = "최소 뎁스 거리", meta = (ClampMin = "100.0", ClampMax = "20000.0"))
	float MinDepthRange = 500.0f;

	// 최소 커널 사이즈입니다. 
	// 커널 사이즈는 엣지를 감지할 때 주변 픽셀을 얼마나 넓게 샘플링할지를 결정합니다.
	// 작을수록 섬세한 엣지를 감지할 수 있고, 클수록 뭉뚱그려진 아웃라인이 생성됩니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SLRendering|Outline|Kernel", DisplayName = "최소 커널 크기", meta = (ClampMin = "1", ClampMax = "30"))
	int32 MinKernelSize = 1.0f;

	// 최대 커널 사이즈입니다. MinKernelSize와 함께 거리 기반으로 보간되며,
	// 멀리 있는 물체일수록 커널이 커지도록 설정해 아웃라인이 눈에 잘 띄게 합니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SLRendering|Outline|Kernel", DisplayName = "최대 커널 크기", meta = (ClampMin = "1", ClampMax = "30"))
...
struct FSolidOutlineSettings : public FBaseOutlineSettings
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SLRendering|Outline|NormalEdge", DisplayName = "노말 엣지 최소 임계값", meta = (ClampMin = "0.0", ClampMax = "1.0"))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SLRendering|Outline|NormalEdge", DisplayName = "노말 엣지 최대 임계값", meta = (ClampMin = "0.0", ClampMax = "1.0"))
	float NormalEdgeMaxThreshold = 0.1f;

	// 뎁스 기반 엣지 감지 시 최소 감지 기준값입니다.
	// 두 픽셀 간 깊이 차이가 이 값보다 작으면 엣지로 간주하지 않습니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SLRendering|Outline|DepthEdge", DisplayName = "깊이 엣지 최소 임계값", meta = (ClampMin = "0.0", ClampMax = "1.0"))
	float DepthEdgeMinThreshold = 0.1f;
	
	// 뎁스 기반 엣지 감지 시 최대 감지 기준값입니다.
	// 두 픽셀 간 깊이 차이가 이 값 이상이면 엣지로 강하게 처리됩니다.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SLRendering|Outline|DepthEdge", DisplayName = "깊이 엣지 최대 임계값", meta = (ClampMin = "0.0", ClampMax = "1.0"))
...
struct FSoftOutlineSettings : public FBaseOutlineSettings
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SLRendering|Outline", DisplayName = "엣지 최소 임계값", meta = (ClampMin = "0.0", ClampMax = "1.0"))
...
private:
	...
	TObjectPtr<UMaterialParameterCollection> MPCSolidOutline;
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "SLRendering", meta = (AllowPrivateAccess = "true"))
	TObjectPtr<UMaterialParameterCollection> MPCSoftOutline;
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "SLRendering", meta = (AllowPrivateAccess = "true"))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SLRendering", DisplayName="솔리드 아웃라인", meta = (AllowPrivateAccess = "true"))
	FSolidOutlineSettings SolidOutlineSettings;
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "SLRendering", DisplayName="소프트 아웃라인", meta = (AllowPrivateAccess = "true"))
