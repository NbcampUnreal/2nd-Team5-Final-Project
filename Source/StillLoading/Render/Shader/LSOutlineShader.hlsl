...
for (float y = -HALF_KERNEL_SIZE; y <= HALF_KERNEL_SIZE; y++)
{
	for (float x = -HALF_KERNEL_SIZE; x <= HALF_KERNEL_SIZE; x++)
	{
		...
		{
			LaplacianFilter_Normal += SceneTextureLookup(PixelUVs, 8, false).rgb * CenterWeight;
			LaplacianFilter_Depth += SceneTextureLookup(PixelUVs, 1, false).r * CenterWeight;
		}
	}
}
